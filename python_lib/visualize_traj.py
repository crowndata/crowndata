...
class TrajectoryPlotter:
    ...
    def plot_trajectory_from_dict(self, trajectory_dict, fig_size=(10, 8)):
        positions, rotations = zip(*[(step_data[:3], step_data[3:]) for step_data in trajectory_dict])
        ...
        for i in range(len(positions)):
            ...
            ax.set(title='End-Effector Trajectory with Rotation in 3D Space', xlabel='X', ylabel='Y', zlabel='Z')
...
def save_video(images, path="trajectory.mp4", fps=15):
    ...
    video_writer = cv2.VideoWriter(path, cv2.VideoWriter_fourcc(*'mp4v'), fps, (width, height))
